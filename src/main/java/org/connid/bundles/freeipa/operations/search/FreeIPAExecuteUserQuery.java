...
public class FreeIPAExecuteUserQuery {

    private static final Log LOG = Log.getLog(FreeIPAExecuteUserQuery.class);
    ...
    public FreeIPAExecuteUserQuery(final LdapFilter query, final ResultsHandler handler,
            final FreeIPAConfiguration freeIPAConfiguration) {
        ldapFilter = query;
    ...
    public void doExecuteQuery() throws LDAPException, GeneralSecurityException, ConnectorException {
        ...
        if (ldapFilter == null || ldapFilter.getNativeFilter() == null) {
            for (final String baseContext : freeIPAConfiguration.getUserBaseContextsToSynchronize()) {
